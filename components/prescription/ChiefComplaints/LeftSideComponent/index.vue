...
const { groupForChiefComplients } = storeToRefs(group);
...
async function search(query: string) {
    if (query === '') return [];
    loading.value = true;
    const result = await searchChiefComplaints(query);
    loading.value = false;
    return result;
}
                ...
                <div v-if="item.key === 'non-group'">
                    <div class="px-0.5">
                        <PrescriptionBaseComponentsSearch
                            ...
                            placeholder="Search/Add Chief Complaints"

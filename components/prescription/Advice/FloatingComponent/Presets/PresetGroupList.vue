...
const { isGroupMode } = storeToRefs(adviceStoreRoot);
const { groupForAdvice } = storeToRefs(prescriptionGroupStore);
...
function onSelectedGroupHandler(group: object) {
    if (Array.isArray(group?.group_item)) {
        group?.group_item.forEach((item: any) => {
            if (!adviceStoreRoot.isAlreadySelected(item?.name)) {
...
function isGroupSelected(group: object) {
    ...
    return group?.group_item.every((item: any) => adviceStoreRoot.isAlreadySelected(item?.name));
}

function onDeleteClickHandler(event: any) {
    prescriptionGroupStore.deleteGroup(event, 'advice');
}

function onEditClickHandler(group: any) {
    adviceStoreRoot.editGroup(group);
}
</script>
...
<template>
    ...
    <div
        :class="
            tailwindMerge(
                'flex max-h-[200px] flex-wrap items-center justify-start gap-2 overflow-auto scrollbar-thin max-1919:h-[176px] max-1599:h-[120px]',

...
const { prescriptionInit, currentTemplateId } = managePrescription();
...
const whenSelectedPatient = async (user: any) => {
    if (route.name === 'dashboard-prescription-create') {
        await prescriptionInit(user);
    ...
    if (route.name === 'patients') {
        navigateTo(`/patients/details/${user.id}`);
    }
};
</script>
    ...
    <section class="py-6">
        <div class="grid h-11 grid-cols-12 gap-8 2xl:h-12">
                            ...
                            <div class="relative w-full cursor-default overflow-hidden">
                                <div class="w-full">
                                    <input
                                        ...
                                        @input="(e) => searchPatientData(e.target.value)"
                                    />
            ...
            <div class="col-span-3">
                <button
                    class="tracking-wides font-poppins-semibold inline-flex h-11 w-full items-center justify-center rounded-md border border-transparent bg-skin-button-accent px-4 text-center text-xs font-semibold text-white transition hover:bg-skin-button-accent-hover focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-300 active:bg-skin-button-accent-hover disabled:opacity-25 2xl:h-12 2xl:text-base"
                    @click="isOpen = true"
                >

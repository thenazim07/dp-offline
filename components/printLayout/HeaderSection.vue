...
const { createHeaderSection } = layout;
const { headerSection } = storeToRefs(layout);
...
const updateInfo = () => {
    ...
    if (updateKey && headerSection.value) {
        const indexToUpdate = headerSection.value.findIndex((item) => item.key === updateKey);

        if (indexToUpdate !== -1) {
            headerSection.value[indexToUpdate].content =
...
const totalWidth = computed(() =>
    headerSection?.value?.filter((item) => item.checked).reduce((sum, column) => sum + Number(column.value), 0),
);

const validateSubmit = computed<boolean>(() => totalWidth.value !== 100 || pending.value === true);
...
<template>
    <div>
        <div class="pb-4">
            <h2 class="pb-4 text-xl font-bold text-gray-900">Header Section</h2>
            <hr />
        </div>

        <div class="mb-6 bg-slate-100">
            <div class="p-4">
                <div class="flex w-[100%] flex-wrap divide-x">
                    <template
                        v-for="(item, index) in headerSection"
                        :key="index"
                    >
                        <div
        ...
        >
            <div class="relative p-8">
                <div class="flex items-center justify-between">
                    <p class="pb-2 text-xl">Edit Header Info</p>
                    <button
                        type="button"
                ...
                <form @submit.prevent="updateInfo">
                    <div class="max-h-[30rem] overflow-y-auto pb-2 scrollbar">

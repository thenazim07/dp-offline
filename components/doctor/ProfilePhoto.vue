...
const { user, changeProfilePhoto } = manageAuth();
...
const getResult = async () => {
    ...
    const photoFile = dataURLtoFile(base64, 'profile.png');
    ...
    <div class="w-52 flex-shrink-0">
        <div class="relative h-36 w-36">
            <NuxtImg
                ...
                class="inline-block h-36 w-36 cursor-pointer rounded-full border object-cover ring-2 ring-transparent ring-white"
                :src="profilePhoto(user)"
                :alt="fullName() ?? ''"
            />
            <button
                class="group absolute -bottom-2 right-6 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200"
                @click="$refs.uploadInput.click()"
            >
                <CameraIcon class="h-5 w-5 text-slate-400 group-hover:text-slate-700" />
                <input
                    ref="uploadInput"
                    type="file"
                    class="hidden"
